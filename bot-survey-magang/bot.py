...
from handlers.prodigi_handlers import ProdigiHandlers
...
async def main_handler(event):
    try:
        ...
        if user_id in potensi_handlers.user_potensi_state:
            ...
            # Koordinat text
            if await potensi_handlers.handle_coordinate_text_with_potensi(
                event,
                user_id
            ):
                return
...
@client.on(events.NewMessage(pattern='/prodigi'))
async def handler(event):
    await prodigi.prodigi_handler(event)
