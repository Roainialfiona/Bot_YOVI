...
class PSHandlers:
    ...
    def _safe_val(self, df, idx, col):
        if col not in df.columns:
            return "-"
        c = df.loc[:, col]
        return c.iloc[idx, 0] if isinstance(c, pd.DataFrame) else c.iloc[idx]
    ...
    async def ps_handler(self, event, tanggal):
        ...
        if "DETAIL KET" in df.columns:
            ...
            status_col = self._safe_col(df, "DETAIL_KET")
            status_name = "DETAIL_KET"

        date_norm = (
            pd.to_datetime(date_col, errors="coerce")
            .dt.strftime("%d/%m/%Y")
        )

        mask = (
            (datel_col.astype(str).str.upper().str.strip() == "BATU") &
            (status_col.astype(str).str.upper().str.strip().isin(["PS", "PS HI"])) &
            (date_norm == tanggal)
        )
